    ...
    const t = setTimeout(async () => {
      try {
        ...
        const itemsPromise = supabase
          .from("items")
          .select("item_id,title,description,location,item_status,available")
          .or(
    ...
    <div className="bg-[#FFFBF2] shadow-md px-4 py-3 md:px-6 flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-0">
      ...
      <div className="flex items-center justify-between md:justify-start gap-3 md:gap-6 w-full md:w-auto">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          <Button
            variant="link"
            className={`${linkClass("notifications")} cursor-pointer relative`}
      ...
      <div className="flex items-center gap-3 md:gap-4 w-full md:w-auto mt-2 md:mt-0">
        <div className="relative w-full md:w-auto">
          ...
          {showSuggest &&
            (suggestions.users.length > 0 || suggestions.items.length > 0) && (
                      ...
                      {suggestions.items.map((it) => (
                        <li key={it.item_id}>
                          <button
                            ...
                            onClick={() =>
                              navigate(
                                `/home?q=${encodeURIComponent(it.title || "")}`
                              )
                            }
                          >
                            <span className="text-sm text-gray-800">
        ...
        <div className="relative">
          <Button
            ...
            className={activePage === "favorites" ? "bg-gray-100" : ""}
          >
            <Heart
          ...
          <DropdownMenuTrigger>
            {user ? (
              <Avatar className="cursor-pointer">
                <AvatarImage
                  src={user.profile_pic_url || user.face_image_url}
                  alt="Profile"
                />
