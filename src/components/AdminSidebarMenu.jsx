...
export default function AdminSidebarMenu() {
  ...
  const mainItems = [{ to: "/adminhome", label: "Admin Dashboard", icon: LayoutDashboard }]
  ...
  const itemManagementItems = [{ to: "/pending-items", label: "Pending Items", icon: PackageSearch }]
  ...
  const renderSection = (items, title) => (
    <div className="space-y-1">
      {title && <p className="px-3 text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">{title}</p>}
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-4 border-t border-gray-200">
        <p className="text-xs text-gray-500 text-center">Admin v1.0.0</p>
      </div>
    </div>
  )
}
