...
import { Loader2, Calendar, Package, User, AudioLines as PhilippinePeso, ChevronRight, Clock } from "lucide-react"
...
function StatusBadge({ status }) {
  ...
  const map = {
    ...
    awaiting_owner_confirmation: "bg-purple-100 text-purple-800 border-purple-200",
  ...
  return <span className={`capitalize border ${cls} px-2 py-0.5 rounded-md text-xs`}>{s.replaceAll("_", " ")}</span>
}

const BOOKING_TABS = [
  { key: "pending", label: "Pending", tip: "Review and approve or reject booking requests." },
  { key: "confirmed", label: "Confirmed", tip: "Waiting for renter to upload deposit proof." },
  { key: "deposit", label: "Deposit", tip: "Review deposit proof and verify to send the item." },
  { key: "onTheWay", label: "On the way", tip: null },
  { key: "ongoing", label: "Ongoing", tip: null },
  { key: "returned", label: "Returned", tip: "Confirm returns and restock items." },
  { key: "cancelled", label: "Cancelled/Rejected", tip: null },
  { key: "expired", label: "Expired", tip: null },
]

export default function OwnerBookingRequests({ favorites, searchTerm, setSearchTerm }) {
  const user = useUser()
  ...
  const combinedDeposits = useMemo(() => {
    const list = [
      ...(awaitingDepositOwner || []).map((r) => ({ ...r, _phase: "awaiting_renter" })),
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const filtered = (data || []).filter((r) => r.renter_id && r.renter_id !== user.id)
      setRows(filtered)
  ...
  const holdReturn = async (txId) => {
    ...
    try {
      ...
      if (error) {
        ...
        toast.info("Return confirmed and item placed on hold (maintenance).")
      }
    } catch (e) {
  ...
  const reportIssue = async (txId) => {
    ...
    try {
      const { error } = await supabase
        ...
        .eq("status", "awaiting_owner_confirmation")
      if (error) {
        toast.error(`Report issue failed: ${error.message || "Unknown error"}`)
        throw error
      } else {
  ...
  const declineDeposit = async (txId) => {
    ...
    try {
      const { error } = await supabase
        ...
        .eq("status", "deposit_submitted")
      if (error) {
        toast.error(`Decline failed: ${error.message || "Unknown error"}`)
        throw error
      } else {
  ...
  const openProof = async (pathOrUrl, title = "Deposit Proof") => {
    try {
      ...
      if (!/^https?:\/\//i.test(pathOrUrl)) {
        const { data, error } = await supabase.storage.from(PROOF_BUCKET).createSignedUrl(pathOrUrl, 300)
        if (error) throw error
        url = data.signedUrl
      }
  ...
  const currentTab = BOOKING_TABS.find((t) => t.key === activeTab) || BOOKING_TABS[0]
  const currentData = grouped[activeTab] || []

  return (
    <div className="min-h-screen bg-[#FAF5EF]">
      <TopMenu activePage="requests" favorites={favorites} searchTerm={searchTerm} setSearchTerm={setSearchTerm} />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
            <div className="space-y-2">
              <h1 className="text-4xl font-bold text-[#1E1E1E] tracking-tight">Booking Requests</h1>
              <p className="text-base text-[#1E1E1E]/60">Manage and track all rental requests in one place</p>
            </div>
            <Button
              variant="outline"
              onClick={fetchData}
          ...
          <CardContent className="p-5">
            <div className="flex items-center justify-between gap-4 flex-wrap">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-[#1E1E1E]">Booking Progress Guide</span>
              </div>
              <ProgressLegend />
            </div>
          </CardContent>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-3xl font-bold text-[#1E1E1E]">{currentTab.label}</h2>
            ...
            {currentData.length === 0 ? (
              <div className="bg-gradient-to-br from-white to-[#FAF5EF]/50 backdrop-blur-sm rounded-2xl border border-[#1E1E1E]/10 p-12 text-center shadow-md">
                ...
                <p className="text-base text-[#1E1E1E]/60 font-medium">
                  No {currentTab.label.toLowerCase()} bookings at the moment.
                </p>
              </div>
            ) : (
      ...
      <ProofPreviewModal open={proofOpen} onOpenChange={setProofOpen} src={proofSrc} title={proofTitle} />
    </div>
  )
}
      ...
      <CardHeader className="pb-4">
        <div className="flex items-start justify-between gap-3">
          <CardTitle className="text-base flex items-center gap-3 flex-1">
            <ImagePreviewThumb src={rental.items?.main_image_url || "/placeholder.svg"} alt={rental.items?.title} />
        ...
        <div className="bg-gradient-to-br from-[#FAF5EF] to-[#FAF5EF]/50 rounded-xl p-4 space-y-3 border border-[#1E1E1E]/5">
          <div className="grid grid-cols-2 gap-x-6 gap-y-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[#1E1E1E]/60 text-xs font-medium">Start Date</span>
            </div>
            <div className="text-[#1E1E1E] font-bold text-xs">{new Date(rental.start_date).toLocaleDateString()}</div>
            <div className="flex items-center gap-2">
              <Calendar className="w-3.5 h-3.5 text-[#FFAB00]" />
              <span className="text-[#1E1E1E]/60 text-xs font-medium">End Date</span>
            </div>
            <div className="text-[#1E1E1E] font-bold text-xs">{new Date(rental.end_date).toLocaleDateString()}</div>
            <div className="flex items-center gap-2">
              <User className="w-3.5 h-3.5 text-[#FFAB00]" />
              <span className="text-[#1E1E1E]/60 text-xs font-medium">Renter</span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[#1E1E1E]/60 text-xs font-medium">Units</span>
            </div>
            <div className="text-[#1E1E1E] font-bold text-xs">{Number(rental.quantity || 1)}</div>
          </div>
          <Separator className="bg-[#1E1E1E]/10" />
          <div className="flex items-center justify-between pt-1">
            <div className="flex items-center gap-2">
              <PhilippinePeso className="w-4 h-4 text-[#FFAB00]" />
              <span className="text-[#1E1E1E]/60 text-xs font-medium">Total Cost</span>
            </div>
            <div className="text-[#FFAB00] font-bold text-lg">₱{Number(rental.total_cost || 0).toFixed(2)}</div>
          </div>
        </div>
  ...
  return (
    <div className="pt-3 flex items-center justify-between gap-2 flex-wrap border-t border-[#1E1E1E]/10">
      <div className="flex gap-2 flex-wrap">
        <RequestDetailsDialog row={rental} awaiting={tabKey === "returned"} />
        ...
        {tabKey === "confirmed" && (
          <span className="text-xs text-[#1E1E1E]/60 py-2">Waiting for renter to upload deposit</span>
        )}
          ...
          <>
            {rental.proof_of_deposit_url && (
              <Button
                ...
                onClick={() => onOpenProof(rental.proof_of_deposit_url, rental.items?.title || "Deposit Proof")}
              >
                View Proof
              </Button>
            )}
            ...
            >
              {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Verify & Send"}
            </Button>
            <Button
              size="sm"
              variant="destructive"
            ...
            >
              {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Decline"}
            </Button>
          </>
        )}

        {tabKey === "deposit" && rental._phase === "awaiting_renter" && (
          <span className="text-xs text-[#1E1E1E]/60 py-2">Awaiting renter upload</span>
        )}
        ...
        {tabKey === "returned" && (
          <>
            {rental.owner_confirmed_at ? (
              ...
              >
                {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Restock Now"}
              </Button>
            ) : (
              <>
                <Button
                  ...
                  disabled={isLoading}
                  onClick={() => onHoldReturn(rental.rental_id)}
                >
                  {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Hold (maintenance)"}
                </Button>
                <Button
                  size="sm"
                  className="bg-green-600 text-white hover:bg-green-700 cursor-pointer"
                  ...
                  onClick={() => onConfirmReturn(rental.rental_id)}
                >
                  {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Confirm & Restock"}
                </Button>
              </>
            )}
            <Button
            ...
            >
              {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Report Issue"}
            </Button>
          </>
        )}
      </div>
        ...
        <div className="space-y-3 text-sm">
          <div className="bg-[#FAF5EF] rounded-lg p-4 space-y-2">
            <div className="flex justify-between">
              <span className="text-[#1E1E1E]/60 font-medium">Renter</span>
              <span className="text-[#1E1E1E] font-semibold">
                {row.renter?.first_name || ""} {row.renter?.last_name || ""}
              </span>
            </div>
          </div>
          <Separator className="bg-[#1E1E1E]/10" />
          <div className="space-y-2">
            <div className="flex justify-between">
              <span className="text-[#1E1E1E]/60 font-medium">Start</span>
              <span className="text-[#1E1E1E] font-semibold">{new Date(row.start_date).toLocaleDateString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[#1E1E1E]/60 font-medium">End</span>
              <span className="text-[#1E1E1E] font-semibold">{new Date(row.end_date).toLocaleDateString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[#1E1E1E]/60 font-medium">Units</span>
              <span className="text-[#1E1E1E] font-semibold">{unitsCount ?? "—"}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-[#1E1E1E]/60 font-medium">Status</span>
            ...
            <div className="flex justify-between items-center pt-2 border-t border-[#1E1E1E]/10">
              ...
              <span className="text-[#FFAB00] font-bold text-lg">₱{Number(row.total_cost || 0).toFixed(2)}</span>
            </div>
          </div>
          {awaiting && (
            <div className="space-y-2 pt-2 border-t border-[#1E1E1E]/10">
              <div className="flex justify-between">
                <span className="text-[#1E1E1E]/60 font-medium">Renter Marked</span>
                <span className="text-[#1E1E1E] font-semibold text-xs">
                  {row.renter_return_marked_at ? new Date(row.renter_return_marked_at).toLocaleString() : "—"}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-[#1E1E1E]/60 font-medium">Owner Confirmed</span>
                <span className="text-[#1E1E1E] font-semibold text-xs">
                  {row.owner_confirmed_at ? new Date(row.owner_confirmed_at).toLocaleString() : "—"}
                </span>
              </div>
            </div>
          )}
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl bg-white">
        <DialogHeader>
          <DialogTitle className="text-[#1E1E1E]">{title || "Deposit Proof"}</DialogTitle>
        </DialogHeader>
        <div className="w-full max-h-[70vh] overflow-auto">
          {src ? (
            isPdf ? (
              <iframe src={src} title={title || "Deposit Proof"} className="w-full h-[70vh] border rounded" />
            ) : (
              <img
            ...
            <div className="text-sm text-gray-600">No proof available.</div>
          )}
        </div>
      </DialogContent>
    </Dialog>
