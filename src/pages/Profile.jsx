...
export default function Profile() {
  ...
  const onUploadFace = async (e) => {
    ...
    try {
      ...
      const { error: upErr } = await supabase.storage.from("user-profile-pic").upload(picPath, file, { upsert: false })
      if (upErr) throw upErr
      const { data: pub } = supabase.storage.from("user-profile-pic").getPublicUrl(picPath)
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      ...
      const withImages = await Promise.all(
        (data || []).map(async (it) => ({
          ...it,
          imageUrl: it.main_image_url || (await getImageUrl(it.user_id, it.item_id)),
        })),
      )
      setItems(withImages)
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-[#FAF5EF]">
      <TopMenu activePage="profile" searchTerm={search} setSearchTerm={setSearch} />
      <div className="max-w-6xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
            ...
            <div className="relative">
              {loadingProfile ? (
                ...
                <Avatar className="h-24 w-24 ring-4 ring-[#FFAB00]/20">
                  <AvatarImage src={profile?.profile_pic_url || ""} alt="Profile" />
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    {loadingRating ? (
                      ...
                      <div className="flex items-center gap-2 bg-[#FFAB00]/10 px-3 py-1.5 rounded-full">
                        ...
                        <span className="font-semibold text-[#1E1E1E]">{rating.average.toFixed(1)}</span>
        ...
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-[#1E1E1E]">Open for booking</h2>
        ...
        {loadingItems ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="border border-[#1E1E1E]/10 rounded-xl overflow-hidden bg-white">
          ...
          <OwnedItemsGrid items={items} isOwner={isOwnProfile} onChanged={loadItems} onRent={onRent} />
        )}
        <BookItemModal
          open={rentOpen}
  ...
  if (!items || items.length === 0) {
    return (
      <div className="text-center py-16 bg-white rounded-xl border border-[#1E1E1E]/10">
        <p className="text-[#1E1E1E]/60 text-lg">No active posts yet.</p>
      </div>
    )
  }
  return (
          ...
          <div className="p-4">
            <div className="flex items-start justify-between gap-2 mb-3">
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold text-[#1E1E1E] text-lg line-clamp-1 mb-1">{it.title}</h3>
                <div className="flex items-baseline gap-1">
                  <span className="text-2xl font-bold text-[#FFAB00]">₱{Number(it.price_per_day || 0).toFixed(0)}</span>
            ...
            <div className="flex items-center justify-between text-xs text-[#1E1E1E]/60 mb-3 pb-3 border-b border-[#1E1E1E]/10">
              <span className="font-medium">
                {Number(it.quantity) || 1} {Number(it.quantity) === 1 ? "unit" : "units"}
              </span>
              {it.location && (
...
function OwnerItemActions({ item, onChanged }) {
  ...
  const updateQuantity = async (delta) => {
    try {
      ...
      const { error } = await supabase.from("items").update({ quantity: next }).eq("item_id", item.item_id)
      if (error) throw error
  ...
  const deletePost = async () => {
    ...
    try {
      setBusy(true)
      const { error } = await supabase.from("items").delete().eq("item_id", item.item_id)
      if (error) throw error
    ...
    <div className="shrink-0">
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button size="icon" variant="ghost" className="cursor-pointer h-8 w-8 hover:bg-[#1E1E1E]/5">
        ...
        <DropdownMenuContent align="end" className="min-w-48">
          ...
          <DropdownMenuItem
            onClick={() => {
              const input = prompt("How many units to subtract?", "1")
              const n = Number.parseInt(input || "0", 10)
              if (!Number.isFinite(n) || n <= 0) return
              updateQuantity(-n)
            }}
          ...
          <DropdownMenuSeparator />
          <DropdownMenuItem data-variant="destructive" onClick={deletePost} disabled={busy}>
            <Trash2 className="w-4 h-4" /> Delete post
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
      <EditItemModal open={editOpen} onOpenChange={setEditOpen} item={item} onSaved={onChanged} />
    </div>
  )
}
  ...
  const save = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        price_per_day: form.price_per_day ? Number.parseFloat(form.price_per_day) : null,
        deposit_fee: form.deposit_fee ? Number.parseFloat(form.deposit_fee) : 0,
        location: form.location.trim() || null,
        available: !!form.available,
        quantity: form.quantity ? Math.max(0, Number.parseInt(form.quantity, 10) || 0) : 0,
      }
      const { error } = await supabase.from("items").update(payload).eq("item_id", item.item_id)
      if (error) throw error
    ...
    <div className="fixed inset-0 bg-[#1E1E1E]/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-2xl font-bold text-[#1E1E1E]">Edit Post</h3>
          <button
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-[#1E1E1E] mb-1.5">Title</label>
            <input
              name="title"
              value={form.title}
              onChange={onChange}
          ...
          <div>
            <label className="block text-sm font-medium text-[#1E1E1E] mb-1.5">Description</label>
            <textarea
              name="description"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-[#1E1E1E] mb-1.5">Price per day (₱)</label>
              <input
                name="price_per_day"
                type="number"
                step="0.01"
            ...
            <div>
              <label className="block text-sm font-medium text-[#1E1E1E] mb-1.5">Deposit (₱)</label>
              <input
                name="deposit_fee"
                type="number"
                step="0.01"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-[#1E1E1E] mb-1.5">Quantity</label>
              <input
                name="quantity"
                type="number"
                step="1"
            ...
            <div>
              <label className="block text-sm font-medium text-[#1E1E1E] mb-1.5">Location</label>
              <input
                name="location"
                value={form.location}
                onChange={onChange}
          ...
          <div className="flex items-center gap-2 pt-2">
            ...
            />
            <label htmlFor="available" className="text-sm font-medium text-[#1E1E1E]">
