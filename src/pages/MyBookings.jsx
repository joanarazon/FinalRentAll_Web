...
import { Clock, Calendar, Package, User, PhilippinePeso, ChevronRight } from "lucide-react"
...
const BOOKING_TABS = [
  ...
  { key: "awaitingDeposit", label: "Confirmed", tip: "Upload your deposit proof to proceed with the booking." },
...
export default function MyBookings() {
  ...
  const grouped = useMemo(() => {
    ...
    for (const r of rows) {
      ...
      else if (st === "cancelled" || st === "rejected") by.cancelled.push(r)
      else if (st === "pending") by.pending.push(r)
      else if (st === "confirmed") by.awaitingDeposit.push(r)
      else if (st === "deposit_submitted") by.awaitingOwnerConfirmDeposit.push(r)
      else if (st === "on_the_way") by.onTheWay.push(r)
      else if (st === "ongoing") by.ongoing.push(r)
      else if (st === "awaiting_owner_confirmation") by.awaitingOwnerConfirmReturn.push(r)
    }
    const term = search.trim().toLowerCase()
    const activeCategory = (categoryFilter || "").trim()
    const filter = (arr) =>
      arr.filter((r) => {
        const titleOk = term ? r.items?.title?.toLowerCase().includes(term) : true
        const catOk = activeCategory ? String(r.items?.category_id || "") === activeCategory : true
  ...
  return (
    <div className="min-h-screen bg-[#FAF5EF]">
      <TopMenu activePage="my-bookings" searchTerm={search} setSearchTerm={setSearch} />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
            <div className="space-y-2">
              <h1 className="text-4xl font-bold text-[#1E1E1E] tracking-tight">My Bookings</h1>
              <p className="text-base text-[#1E1E1E]/60">Manage and track all your rental bookings in one place</p>
            </div>
            <div className="flex items-center gap-3 bg-white rounded-xl px-5 py-3.5 shadow-sm border border-[#1E1E1E]/10 hover:shadow-md transition-shadow">
              ...
              <label className="text-sm font-semibold text-[#1E1E1E]/70">Category</label>
              <select
                ...
                {currentData.map((r) => (
                  ...
                  >
                    ...
                    <CardContent className="text-sm space-y-5">
                      ...
                      <div className="bg-gradient-to-br from-[#FAF5EF] to-[#FAF5EF]/50 rounded-xl p-4 space-y-3 border border-[#1E1E1E]/5">
                        <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[#1E1E1E]/60 text-xs font-medium">Owner</span>
                          </div>
                          <div className="text-[#1E1E1E] font-bold text-xs">
                          ...
                          <div className="text-[#1E1E1E] font-bold text-xs">{Number(r.quantity || 1)}</div>
                        </div>
                      ...
                      <ActionBar tabKey={activeTab} rental={r} user={user} onChanged={refetchBookings} />
                    </CardContent>
                  </Card>
                ))}
              </div>
...
function ActionBar({ tabKey, rental, user, onChanged }) {
  let primary = null
  if (tabKey === "pending") {
    primary = <CancelBooking rental={rental} onChanged={onChanged} user={user} />
...
function MoreMenu({ rental, user, onChanged }) {
  ...
  const openProof = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.storage.from(PROOF_BUCKET).createSignedUrl(url, 300)
      if (error) throw error
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="bg-white">
          ...
          <DropdownMenuSeparator />
          <ReportDialog
            trigger={<DropdownMenuItem>Report owner</DropdownMenuItem>}
      ...
      <Dialog open={openUpload} onOpenChange={setOpenUpload}>
        <DialogContent className="bg-white">
          <DialogHeader>
            <DialogTitle className="text-[#1E1E1E]">Upload deposit proof</DialogTitle>
          </DialogHeader>
...
function CancelBooking({ rental, onChanged, user }) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="bg-white">
        <DialogHeader>
          <DialogTitle className="text-[#1E1E1E]">Cancel booking request?</DialogTitle>
        </DialogHeader>
        <div className="text-sm text-[#1E1E1E]/80 space-y-3">
          <p>
            This will withdraw your request for
            <span className="font-semibold text-[#1E1E1E]"> {rental.items?.title}</span>.
          </p>
          ...
          <p className="text-xs text-[#1E1E1E]/50">This action cannot be undone.</p>
        </div>
        <DialogFooter>
          <Button
            type="button"
...
function isEligibleReturn(rental) {
  ...
  return end <= now && ["confirmed", "ongoing"].includes(String(rental.status || "").toLowerCase())
}
...
function MarkReturned({ rental, onChanged }) {
  ...
  const doMark = async () => {
    try {
      ...
      toast.error(`Could not mark returned: ${e.message || "Unknown error"}`)
    } finally {
      setSubmitting(false)
    }
  }
...
function RateLessorButton({ rental }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        setLoading(true)
        const data = await getExistingLessorReview(rental.rental_id, user?.id)
        setExisting(data)
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="bg-white">
        <DialogHeader>
          <DialogTitle className="text-[#1E1E1E]">Rate lessor</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-3 text-sm">
          <div className="bg-[#FAF5EF] rounded-lg p-4 space-y-2">
            <div className="flex justify-between">
              <span className="text-[#1E1E1E]/60 font-medium">Owner</span>
              <span className="text-[#1E1E1E] font-semibold">
                {rental.items?.owner?.first_name || ""} {rental.items?.owner?.last_name || ""}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-[#1E1E1E]/60 font-medium">Renter</span>
              <span className="text-[#1E1E1E] font-semibold">
                {rental.renter?.first_name || "You"} {rental.renter?.last_name || ""}
              </span>
            </div>
          </div>
          <Separator className="bg-[#1E1E1E]/10" />
          <div className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="text-[#1E1E1E] font-semibold">{new Date(rental.start_date).toLocaleDateString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[#1E1E1E]/60 font-medium">End</span>
              <span className="text-[#1E1E1E] font-semibold">{new Date(rental.end_date).toLocaleDateString()}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between items-center pt-2 border-t border-[#1E1E1E]/10">
              ...
              <span className="text-[#FFAB00] font-bold text-lg">₱{Number(rental.total_cost || 0).toFixed(2)}</span>
            </div>
          </div>
          <div className="pt-2">
            <p className="text-[#1E1E1E]/60 font-medium text-xs mb-2">Booking Progress</p>
            <BookingSteps status={String(rental.status || "")} compact />
          </div>
        </div>
...
function UploadDeposit({ rental, onChanged }) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="sm" className="cursor-pointer bg-[#FFAB00] text-[#1E1E1E] hover:bg-[#FFAB00]/90 font-semibold">
          Upload Deposit
        </Button>
      </DialogTrigger>
      <DialogContent className="bg-white">
        ...
        <form onSubmit={handleUpload} className="space-y-4">
          <div className="border-2 border-dashed border-[#1E1E1E]/20 rounded-lg p-6 text-center hover:border-[#FFAB00] transition-all">
            <input
              ...
              className="w-full text-sm text-[#1E1E1E] file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-[#FFAB00] file:text-[#1E1E1E] hover:file:bg-[#FFAB00]/90 file:cursor-pointer"
            />
            {file && <p className="mt-3 text-xs text-[#1E1E1E]/60">Selected: {file.name}</p>}
          </div>
          <DialogFooter>
            <Button
              type="button"
